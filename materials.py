...
class CheXpertTrainer():
    def train(model, dataLoaderTrain, dataLoaderVal, class_names, nnClassCount, trMaxEpoch, PATH, f_or_l, checkpoint, cfg):
        ...
        lossv1, lossv_each = CheXpertTrainer.epochVal(
            model, dataLoaderVal, optimizer, trMaxEpoch, nnClassCount, loss)
        print("Initial valid loss (overall): {:.3f}".format(lossv1))
        for i in range(5):
            print("Initial valid loss {}: {:.3f}".format(
                class_names[i], lossv_each[i]))
        print('')
        ...
        for epochID in range(0, trMaxEpoch):
            ...
            losst = CheXpertTrainer.epochTrain(
                model, dataLoaderTrain, optimizer, nnClassCount, loss, PATH, f_or_l)
            train_end.append(time.time())   # training ends

            lossv, lossv_each = CheXpertTrainer.epochVal(
                model, dataLoaderVal, optimizer, trMaxEpoch, nnClassCount, loss)
            for i in range(5):
                ...
                if lossv_each[i] < lossMIN_each[i]:
                    ...
                    print('Epoch ' + str(epochID + 1) +
                          ' [IMPR] lossv {} = {:.3f}'.format(class_names[i], lossv_each[i]))
                else:
                    print('Epoch ' + str(epochID + 1) +
                          ' [----] lossv {} = {:.3f}'.format(class_names[i], lossv_each[i]))

            if lossv < lossMIN:
                lossMIN = lossv
                model_num = epochID + 1
                print('Epoch ' + str(epochID + 1) +
                      ' [IMPR] loss = {:.3f}'.format(lossv))
            else:
                print('Epoch ' + str(epochID + 1) +
                      ' [----] loss = {:.3f}'.format(lossv))

            print("Training loss: {:.3f},".format(losst),
                  "Valid loss: {:.3f}".format(lossv))
    ...
    def epochVal(model, dataLoaderVal, optimizer, trMaxEpoch, nnClassCount, loss):
        ...
        with torch.no_grad():
            for i, (varInput, target) in enumerate(dataLoaderVal):
                ...
                varOutput_Card = torch.tensor(
                    [i[0] for i in varOutput.tolist()])
                target_Card = torch.tensor([i[0] for i in target.tolist()])
                varOutput_Edem = torch.tensor(
                    [i[1] for i in varOutput.tolist()])
                target_Edem = torch.tensor([i[1] for i in target.tolist()])
                varOutput_Cons = torch.tensor(
                    [i[2] for i in varOutput.tolist()])
                target_Cons = torch.tensor([i[2] for i in target.tolist()])
                varOutput_Atel = torch.tensor(
                    [i[3] for i in varOutput.tolist()])
                target_Atel = torch.tensor([i[3] for i in target.tolist()])
                varOutput_PlEf = torch.tensor(
                    [i[4] for i in varOutput.tolist()])
                target_PlEf = torch.tensor([i[4] for i in target.tolist()])

                lossvalue = loss(varOutput, target)
                lossVal += lossvalue.item()*varInput.size(0)

                lossVal_Card += loss(varOutput_Card,
                                     target_Card).item()*varInput.size(0)
                lossVal_Edem += loss(varOutput_Edem,
                                     target_Edem).item()*varInput.size(0)
                lossVal_Cons += loss(varOutput_Cons,
                                     target_Cons).item()*varInput.size(0)
                lossVal_Atel += loss(varOutput_Atel,
                                     target_Atel).item()*varInput.size(0)
                lossVal_PlEf += loss(varOutput_PlEf,
                                     target_PlEf).item()*varInput.size(0)
            ...
            lossv_each = [lossv_Card, lossv_Edem,
                          lossv_Cons, lossv_Atel, lossv_PlEf]
    ...
    def test(model, dataLoaderTest, nnClassCount, checkpoint, class_names, f_or_l):
        ...
        aurocIndividual = CheXpertTrainer.computeAUROC(
